...
export default class RestaurantController implements Crud {
    ...
    async create(request: Request, response: Response){
        const {name, adress, foodStyle} = request.body;

        const isAnyUndefined = Citi.areValuesUndefined(name, adress, foodStyle);
        if(isAnyUndefined) return response.status(400).send();

        const newRestaurant = { name, adress, foodStyle };
    ...
    async update(request: Request, response: Response){
        ...
        const isAnyUndefined = Citi.areValuesUndefined(name, adress, foodStyle, id);
        if(isAnyUndefined) return response.status(400).send();

        const restaurantWithUpdatedValues = {name, adress, foodStyle};
